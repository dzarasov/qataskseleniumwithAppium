...
public class MainCl {
	...
	public MainCl(WebDriver driver){
		this.driver = driver;
	}
	...
	public ResultsP navigateToResultsPage(String state){
		selectStateValue(state);
		checkoutButton().click();
		return new ResultsP(driver);
	}
	...
	public double[] pricesTotals(double state){
    	...
    	int zebraTotalPrice = getTotalPrice(MainCl.ZEBRAID, priceAndQuantity().get(MainCl.expectedZebraValue)[0],   priceAndQuantity().get(MainCl.expectedZebraValue)[1], 1);
    	int lionTotalPrice  = getTotalPrice(MainCl.LIONID,  priceAndQuantity().get(MainCl.expectedLionValue)[0],    priceAndQuantity().get(MainCl.expectedLionValue)[1], 1);
    	int elephTotalPrice = getTotalPrice(MainCl.ELEPHID, priceAndQuantity().get(MainCl.expectedElephantValue)[0],priceAndQuantity().get(MainCl.expectedElephantValue)[1], 1);
    	int giraffTotalPrice= getTotalPrice(MainCl.GIRAFFID,priceAndQuantity().get(MainCl.expectedGiraffeValue)[0], priceAndQuantity().get(MainCl.expectedGiraffeValue)[1], 1);
